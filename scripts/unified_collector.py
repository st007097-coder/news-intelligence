#!/usr/bin/env python3
"""
Unified Collector: Tavily Search + RSS Feeds
Combines all phases into single pipeline
"""
...
def search_tavily(query: str, api_key: str = None) -> list:
    ...
    if not api_key:
        ...
        config_file = os.path.join(os.path.dirname(__file__), '..', 'sources', 'tavily_config.json')
...
def unified_pipeline():
    ...
    print("=" * 60)
    print("🚀 News Intelligence — Unified Pipeline")
    print("=" * 60)
    
    all_items = []
    
    # Step 1: Cleanup old events
    print("\n[1/5] 🧹 Cleanup old events...")
    ...
    print(f"  RSS total: {len(rss_items)} items")
    
    # Step 3: Search Tavily
    print("\n[3/5] 🔍 Search Tavily...")
    # Note: Tavily API requires key. Skip if not available.
    tavily_items = []
    ...
    if tavily_items:
        print(f"  Tavily total: {len(tavily_items)} items")
